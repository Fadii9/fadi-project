...
const Customers: React.FC<{ time: number }> = ({ time }) => {
  ...
  const shortestQueue = allQueues.reduce(function (p, c) {
    return p.length > c.length ? c : p;
  }); //find shortest queue
  const longestQueue = allQueues.reduce(function (p, c) {
    return p.length < c.length ? c : p;
  }); //find longest queue

  //check if there is an available space in the queues
  const availableQueue =
    queue1.length < fullQueue ||
    queue2.length < fullQueue ||
    queue3.length < fullQueue;
